...
async function initFirebase() {
  ...
  try {
    /* SDKs already loaded via <script> tags — just initialize */
    ...
    if (typeof firebase.auth === 'function') {
      ...
      try {
        ...
        console.warn('Anonymous auth:', ae.message);
      }
    }
...
/* ================================================
   GET USER LIST  (visitors with names)
================================================ */
async function dbGetUserList() {
  if (!DB) return [];
  try {
    const snap = await DB
      .collection('visitors')
    ...
    console.warn('dbGetUserList:', e.message);
    return [];
  }
}
...
/* ================================================
   FEEDBACK / MESSAGES
================================================ */
async function dbSendFeedback(data) {
  if (!DB) return false;
  try {
    ...
    console.warn('dbSendFeedback:', e.message);
    return false;
  }
}

async function dbGetFeedback(limit = 50) {
  if (!DB) return [];
  try {
    const snap = await DB
      .collection('feedback')
    ...
    console.warn('dbGetFeedback:', e.message);
    return [];
  }
}

async function dbMarkFeedbackRead(id) {
  if (!DB) return;
  try {
