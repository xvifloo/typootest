...
/* ──────────────────────────────────────────────
   WPM CALCULATIONS — CORRECT FORMULA
   
   Gross WPM = (Total chars typed / 5) / minutes
   Net  WPM  = Gross WPM − (Errors / minutes)
   
   Standard: 5 characters = 1 "word"
   We use Net WPM as the main display value.
────────────────────────────────────────────── */

function calcGrossWPM() {
  if (!S.t0) return 0;
  const mins = (Date.now() - S.t0) / 60000;
  if (mins < 0.01) return 0;
  /* Total characters attempted / 5 / minutes */
  return Math.max(0, Math.round((S.totalKS / 5) / mins));
}

function calcWPM() {
  /* Net WPM = (Correct chars / 5) / minutes
     This is the standard competitive typing formula.
     Errors are implicitly excluded because we only
     count correctKS (keystrokes that matched). */
  if (!S.t0) return 0;
  const mins = (Date.now() - S.t0) / 60000;
  if (mins < 0.01) return 0;
  const net = Math.max(0, Math.round((S.correctKS / 5) / mins));
  return net;
}

/* Accuracy — correct / total typed (not text length) */
function calcAcc() {
  if (S.totalKS === 0) return 100;
  const pct = (S.correctKS / S.totalKS) * 100;
  /* Clamp 0–100, round to 1 decimal then floor for display */
  return Math.max(0, Math.min(100, Math.round(pct)));
}

/* Characters Per Minute — correct chars only */
function calcCPM() {
  if (!S.t0) return 0;
  const mins = (Date.now() - S.t0) / 60000;
  if (mins < 0.01) return 0;
...
function updateLessonsProgress() {
  ...
  const list = listMap[tab] || S.lessonsEN || [];
  const total = list.reduce((sum, lesson) => sum + ((lesson.parts && lesson.parts.length) ? lesson.parts.length : 0), 0);
  let   done  = 0;

  list.forEach(lesson => {
    const partCount = (lesson.parts && lesson.parts.length) ? lesson.parts.length : 0;
    for (let i = 1; i <= partCount; i++) {
      if (storageIsPartDone(lesson.id, i)) done++;
    }
  });
  ...
  if (label) {
    const doneLabel = (typeof t === 'function') ? t('donePartsLabel') : 'parts done';
